...
export class ClientCaptureService {
    ...
    public onReceivedCaptureID: Function;
    ...
    private apiUrl = "http://admin.dev.stefanka.tech/api/"; //"http://localhost/api/";
    private apiAuthorization = 'Basic cGFuZHc6cGFuZHdhZG1pbg==';
    ...
    checkOut_clientEmail(email) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
    ...
    // when the client press suivant or Incognito button this function will be called 
    //to register the client 
    // returns clientId
    register_client(email, fName, lName, anonymous) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
        let options = new RequestOptions({ headers: headers });
        let body = "";
        let jsonToSend;
    ...
    // in fitting room when the clients enter their email 
    ...
    //register breast type 
    register_breast(clientId,captureId,breastType) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
        let options = new RequestOptions({ headers: headers });
        let jsonToSend = {
            "client_id": clientId,
        ...
        let body = JSON.stringify(jsonToSend);
        //console.log(body);
        return this.http.post(this.apiUrl + "clientCaptures/edit/" + captureId + ".json", body, options)
            .subscribe(
            (res: Response) => this.receivedResultRegBreastType(res),
            (err) => this.catchError()  // catching exception 
            );
    }
    ...
    // returns captureId
    register_measurement(bust, thorax, clientId) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
        let options = new RequestOptions({ headers: headers });
        let capture = {
            "type": "Manual",
            ...
            "client_capture_metrics": [
                {
                    "type": "breast_band_size",
                    "value": bust
                },
                {
                    "type": "under-breast_band_size",
                    "value": thorax
                }
            ]
        };
    ...
    private receivedCaptureID(res: Response) {
        ...
        this.onReceivedCaptureID();
    }
    public setReceivedCaptureIDCallback(cb: Function) {
        this.onReceivedCaptureID = cb;
    }
    // for finding the products which are match to client's measurment
    findMatchProduct(captureId) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
        let options = new RequestOptions({ headers: headers });
        let jsonToSend = { "garment_type": "bra", "capture_id": captureId };
    ...
    receivedResultRegBreastType(res: Response) {
    }
    ...
    private createProducts() {
        ...
        for (var i = 0; i < this.products.length; i++) {
            try {
                ...
                this.prdBrand = this.getVersionMetadata("manufacturer", i, 0);
                this.prdCut = this.getVersionMetadata("collection", i, 0);
                this.prdSize = this.getVersionMetadata("_bra_size", i, 0);  //????????????
