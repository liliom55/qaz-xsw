...
export class CatalogComponent implements AfterViewInit {
    ...
    constructor(private addTocartService: AddToCartService, private router: Router,
        ...
        if (JSON.parse(sessionStorage.getItem("products")) != null) {
            ...
            if (JSON.parse(sessionStorage.getItem("orderNumber")) !== null) {
                this.addTocartService.listCartProduct = JSON.parse(sessionStorage.getItem("listCartProduct"));
                this.addTocartService.orderNumber = parseInt(sessionStorage.getItem("orderNumber"));
            } else {
        ...
        sessionStorage.setItem("listCartProduct", JSON.stringify(this.addTocartService.listCartProduct));
    ...
    createRelation() {
        for (var i = 0; i < this.addTocartService.listCartProduct.length; i++) {
            this.listPrdVersionIdSelected.push(this.addTocartService.listCartProduct[i].versionId);
        }
    ...
    sendEmail() {
        if (sessionStorage.getItem("email") != "" && sessionStorage.getItem("email") != null) {
            this.clientCaptureService.sendEmail(this.addTocartService.listCartProduct);
        }
    }
    ...
    //swipe event for closing product_panel in mobile/ipad
    private ww;
    private xx;
    swipe(action) {
    ...
    ngAfterViewInit() {
        ...
        //Btn Filter
        var is_open = false;
        //$(this.btn_filter.nativeElement).on('click touch', (e, args) => {
        ...
        $("#passer label").on('click touch', (e, args) => {
            if (this.addTocartService.orderNumber > 0) {
