...
export class ClientCaptureService {
    ...
    private apiUrl = 'http://localhost/api/';  // "https://fitadmin.ces.stefanka.tech/api/"; //
    ...
    // returns captureId
    register_measurement(bust, thorax,breastType, clientId) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
        let options = new RequestOptions({ headers: headers });
        let capture = {
            "type": "Manual",
    ...
    public sendEmail(selected_prd) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': 'Basic c2ltb25zYXBpOnNpbW9uc2F5cw==' });
        let options = new RequestOptions({ headers: headers });
        let jsonToSend = {};
    ...
    private bindClientToCapture() {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this.apiAuthorization });
        let options = new RequestOptions({ headers: headers });
        let jsonToSend = { "client_id": this.clientId };
        let body = JSON.stringify(jsonToSend);
