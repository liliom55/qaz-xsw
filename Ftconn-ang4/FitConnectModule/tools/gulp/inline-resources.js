...
function inlineResources(projectPath) {
  ...
  return Promise.all(files.map(filePath => {
    ...
    return readFile(fullFilePath, 'utf-8')
      .then(content => inlineResourcesFromString(content, fullFilePath, url => {
 ...
 */
function inlineResourcesFromString(content, fullFilePath, urlResolver) {
  // Curry through the inlining functions.
  return [
    inlineTemplate,
    inlineTemplateUrlKeywords,
    inlineStyle,
    inlineStyleUrlKeywords,
    removeModuleId
  ].reduce((content, fn) => fn(content, fullFilePath, urlResolver), content);
}
 ...
 */
function inlineTemplate(content, fullFilePath, urlResolver) {
  return content.replace(/'([^']+?\.html)'/g, function (m, templateUrl) {
    let filename = path.basename(templateUrl);
    console.log("filename: " + filename);
    const templateFile = findFile(path.dirname(fullFilePath), filename, urlResolver);
    console.log(templateFile);
    const templateContent = fs.readFileSync(templateFile, 'utf-8');
    const shortenedTemplate = templateContent
      .replace(/([\n\r]\s*)+/gm, ' ')
      .replace(/"/g, '\\"');
    return `"${shortenedTemplate}"`;
  });
}

function findFile(root, fileName, urlResolver) {
  ...
  if (fs.existsSync(root + '/' + fileName)) {
    ...
    var dirs = fs.readdirSync(root).filter(f => fs.statSync(root + "/" + f).isDirectory());
    dirs.some(function (element) {
      console.log("folder " + element);
      file = findFile(path.join(root, element), fileName, urlResolver);
      if (file) {
...
function inlineTemplateUrlKeywords(content, fullFilePath, urlResolver) {
  return content.replace(/templateUrl/g, function (m, templateUrl) {
    return `template`;
  });
}
 ...
 */
function inlineStyle(content, fullFilePath, urlResolver) {
  return content.replace(/'([^']+?\.scss)'/g, function (m, styleUrl) {
    let filename = path.basename(styleUrl);
    const styleFile = findFile(path.dirname(fullFilePath), filename, urlResolver);
...
function inlineStyleUrlKeywords(content, fullFilePath, urlResolver) {
  return content.replace(/styleUrls/g, function (m, templateUrl) {
    return `styles`;
  });
}
